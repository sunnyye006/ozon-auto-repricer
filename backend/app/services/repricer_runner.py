...
class RepricerRunner:
    ...
    async def run_for_store(self, db: AsyncSession, store: Store) -> None:
        ...
        for product in store.products:
            if not product.auto_reprice_enabled or product.current_price <= 0:
                continue
        ...
        store.last_scanned_at = datetime.now(timezone.utc)

    async def run_once(self) -> None:
        async with SessionLocal() as db:
            stores = await db.scalars(
                select(Store)
                .where(Store.is_active.is_(True), Store.auto_reprice_enabled.is_(True))
                .options(selectinload(Store.products).selectinload(Product.state))
            )
            for store in stores.all():
                await self.run_for_store(db, store)
            await db.commit()
